(() => {
  ...
  const copy = {
    ...
    ru: {
      language: "Язык", chooseLanguage: "Выберите язык", languageIntro: "Его можно изменить в любой момент в верхней панели.", menu: "Меню", home: "Главная", projects: "Проекты", explore: "Исследовать", about: "About", aboutOne: "Panda Project — это лаборатория абсурдных и немного сумбурных проектов.", aboutTwo: "Идеи создаются без тщательного планирования, специальных навыков или крупных вложений: начинаешь с того, что есть, а всему остальному учишься по пути.", aboutThree: "Он появился как способ противостоять жизни, уже расписанной заранее, где желаемое постоянно откладывается в ожидании подходящего момента.", aboutFour: "Название пришло от первого проекта, который действительно воплотился в жизнь: Fiat Panda 4×4 1998 года, восстановленной без каких-либо механических навыков, между дневной работой и вечерними сменами за барной стойкой.", aboutFive: "Но Panda Project не начинается и не заканчивается машиной или путешествием.", aboutSix: "Panda — лишь первая из тысяч идей, продержавшаяся достаточно долго, чтобы стать чем-то реальным.", aboutSeven: "Общая идея проста: относиться к сомнительной задумке серьёзно достаточно долго, чтобы узнать, чем она может стать.", aboutEight: "Или сколько проблем она может вызвать.", maps: "Карты", projectTitle: "Бюджетный<br>оверлендинг на<br><em>неподходящих<br>автомобилях.</em>", showProject: "Открыть проект", build: "Проект в работе", soon: "Скоро", pandaModel: "3D-модель Panda Anna", pandaFallback: "3D-модель не загрузилась. Перезагрузите страницу или откройте сайт в браузере.", vehicleSelection: "Выбор автомобиля", controls: "Управление выбором автомобиля", chooseVehicle: "Выберите автомобиль",
