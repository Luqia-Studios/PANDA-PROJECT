(() => {
  ...
  const copy = {
    ...
    de: {
      language: "Sprache", chooseLanguage: "Sprache wählen", languageIntro: "Du kannst sie jederzeit über die obere Leiste ändern.", menu: "Menü", home: "Home", projects: "Projekte", explore: "Entdecken", support: "Projekt unterstützen", maps: "Maps", projectTitle: "Low-budget<br>overland in<br><em>inappropriate<br>vehicles.</em>", showProject: "Show the Project", build: "Build in progress", soon: "Coming soon", pandaModel: "3D-Modell von Panda Anna", pandaFallback: "Das 3D-Modell wurde nicht geladen. Lade die Seite neu oder öffne die Vorschau im Browser.", vehicleSelection: "Fahrzeugauswahl", controls: "Steuerung der Fahrzeugauswahl", chooseVehicle: "Fahrzeug auswählen",
  ...
  const css = `
    ...
    .language-modal{position:fixed;z-index:100;inset:0;display:grid;place-items:center;padding:20px;background:rgba(36,35,34,.78);backdrop-filter:blur(5px)}.language-modal[hidden]{display:none}.language-modal__panel{position:relative;width:min(420px,100%);padding:clamp(25px,5vw,46px);border:1px solid var(--line);background:var(--paper);box-shadow:0 18px 48px rgba(0,0,0,.35)}.language-modal__close{position:absolute;top:14px;right:16px;border:0;background:transparent;color:var(--acid);font-size:26px;cursor:pointer}.language-modal h2{font-size:clamp(38px,8vw,62px)}.language-modal p{max-width:28ch;margin:18px 0 30px;color:var(--muted);font-size:13px;line-height:1.5}.language-options{display:grid;gap:9px}.language-option{display:flex;align-items:center;justify-content:space-between;min-height:54px;padding:12px 14px;border:1px solid var(--line);background:transparent;color:var(--ink);font:500 18px "Space Grotesk",sans-serif;letter-spacing:-.04em;cursor:pointer}.language-option:hover,.language-option.is-active{border-color:var(--acid);color:var(--acid)}.language-option span{color:var(--muted);font:600 10px Poppins,sans-serif;letter-spacing:.1em;text-transform:uppercase}@media(max-width:680px){.quick-nav>.header-actions{gap:10px}.social-links{gap:6px}.social-button{width:34px;height:34px}.social-button svg{width:19px;height:19px}.map-back,.language-toggle,.quick-nav .menu-button{height:34px;min-height:34px}.language-toggle{gap:0;font-size:0}.language-toggle strong{font-size:12px}.language-modal__panel{padding:28px 22px}.language-modal p{margin-block:14px 22px}}
  `;
